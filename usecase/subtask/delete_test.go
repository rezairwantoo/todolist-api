...
var _ = Describe("Get Detail Task", func() {
	...
	Describe("Get Detail Usecase", func() {
		Context("When request for get detail", func() {
			It("success", func() {
				req := model.DeleteSubTaskRequest{
					ID:     2,
					TaskID: 1,
				}
				...
				mockTaskRepo.EXPECT().DeleteTask(ctx, req.ID).Return(nil)
			...
			It("error when deleting data", func() {
				req := model.DeleteSubTaskRequest{
					ID:     2,
					TaskID: 1,
				}
				...
				reqDetail.TaskID = 2
				mockTaskRepo.EXPECT().DetailTask(ctx, reqDetail).Return(subtask, nil)
				mockTaskRepo.EXPECT().DeleteTask(ctx, req.ID).Return(err)
				_, err := mockUsecase.Delete(ctx, req)
				Expect(err).ToNot(BeNil())
			})
		})
